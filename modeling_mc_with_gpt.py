...
class GPT2Attention(nn.Module):
    def __init__(self, config, is_cross_attention=False, layer_idx=None, 
        ...
        self.next_token_q_attn = None
        self.next_token_c_proj = None
        if self.indi_query:
            print("make next token q_proj layer")
            self.next_token_q_attn = Conv1D(self.embed_dim, self.embed_dim)
        if self.indi_output:
    ...
    def forward(
        ...
        #code for proposed methods
        elif is_next_token and self.indi_query:
            query = self.q_attn(hidden_states)
            key, value = self.c_attn(encoder_hidden_states).split(self.split_size, dim=2)
        else:
        ...
        #code for proposed methods
        if is_next_token and self.indi_output:
            attn_output = self.next_token_c_proj(attn_output)
        else:
...
class GPT2Block(nn.Module):
    def __init__(self, config, layer_idx=None):
        ...
        self.attn = attention_class(config=config, layer_idx=layer_idx, indi_query=config.indi_self_query, indi_output=config.indi_self_output,)
        ...
        if config.share_self_attention_module:
            ...
            self.next_token_attn = self.attn
        else:
            self.attn = attention_class(config=config, layer_idx=layer_idx)
        
        if config.share_ffnn:
            print("shared ffnn")
            self.next_token_mlp = self.mlp
            self.next_token_ln_1 = self.ln_1
            self.next_token_ln_2 = self.ln_2
        else:
            print("not shared ffnn")
            self.next_token_mlp = GPT2MLP(inner_dim, config)
            self.next_token_ln_1 = nn.LayerNorm(hidden_size, eps=config.layer_norm_epsilon)
            self.next_token_ln_2 = nn.LayerNorm(hidden_size, eps=config.layer_norm_epsilon)


    def forward(
        self,
        hidden_states: Optional[Tuple[torch.FloatTensor]],
        next_token_hidden_states: Optional[Tuple[torch.FloatTensor]],
        layer_past: Optional[Tuple[torch.Tensor]] = None,
        attention_mask: Optional[torch.FloatTensor] = None,
        head_mask: Optional[torch.FloatTensor] = None,
        encoder_hidden_states: Optional[torch.Tensor] = None,
        ...
        next_token_hidden_states = self.next_token_ln_1(next_token_hidden_states)
        next_token_attn_outputs = self.next_token_attn(
        ...
        next_token_attn_output = next_token_attn_outputs[0]  # output_attn: a, present, (attentions)
        next_token_outputs = next_token_attn_outputs[1:]
        # residual connection
        next_token_hidden_states = next_token_attn_output + next_token_residual
        ###
        ...
        #code for proposed methods
        next_token_residual = next_token_hidden_states
        next_token_hidden_states = self.next_token_ln_2(next_token_hidden_states)
        next_token_feed_forward_hidden_states = self.next_token_mlp(next_token_hidden_states)
        # residual connection
        next_token_hidden_states = next_token_residual + next_token_feed_forward_hidden_states
        ###
...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        #code for proposed methods
        next_token_positions = self.wpe(position_ids + 1)
        ...
        next_token_hidden_state = self.drop(next_token_hidden_state)
        ...
        for i, (block, layer_past) in enumerate(zip(self.h, past_key_values)):
            ...
            next_token_hidden_state = outputs[-1]
            if use_cache is True:
        ...
        next_token_hidden_state = self.ln_f(next_token_hidden_state)
        next_token_hidden_state = next_token_hidden_state.view(output_shape)
