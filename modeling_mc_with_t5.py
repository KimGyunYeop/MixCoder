...
class T5Attention(nn.Module):
    def __init__(self, config: T5Config, 
        ...
        if config.is_decoder:
            ...
            if self.indi_query:
                ...
                self.next_token_q = nn.Linear(self.d_model, self.inner_dim, bias=False)
            if self.indi_output:
                print("make next token out_proj layer")
                self.next_token_o = nn.Linear(self.inner_dim, self.d_model, bias=False)
    ...
    #code for proposed methods
    def compute_bias_for_next_token(self, query_length, key_length, device=None):
        """Compute binned relative position bias"""
        if device is None:
            device = self.relative_attention_bias.weight.device
        context_position = torch.arange(1, query_length+1, dtype=torch.long, device=device)[:, None]
    ...
    def forward(
        ...
        if self.is_decoder:
            if self.indi_query and is_next_token:
                query_states = shape(self.next_token_q(hidden_states))
            else:
                # get query proj
        ...
        if position_bias is None:
            if not self.has_relative_attention_bias:
                ...
                #code for proposed methods
                if is_next_token:
                    position_bias = self.compute_bias_for_next_token(real_seq_length, key_length, device=scores.device)
                else:
        ...
        if self.is_decoder:
            if self.indi_output and is_next_token:
                attn_output = self.next_token_o(attn_output)
            else:
...
class T5LayerSelfAttention(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False,
        ...
        super().__init__()
        self.SelfAttention = T5Attention(config, has_relative_attention_bias=has_relative_attention_bias, indi_query=indi_query, indi_output=indi_output)
        self.layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
...
class T5LayerCrossAttention(nn.Module):
    def __init__(self, config, indi_query: bool = False, indi_output: bool = False,):
        super().__init__()
        self.EncDecAttention = T5Attention(config, has_relative_attention_bias=False, indi_query=indi_query, indi_output=indi_output)
        self.layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
...
class T5Block(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False):
        ...
        self.layer.append(T5LayerSelfAttention(config, has_relative_attention_bias=has_relative_attention_bias, indi_query=config.indi_self_query, indi_output=config.indi_self_output))
        if self.is_decoder:
            self.layer.append(T5LayerCrossAttention(config, indi_query=config.indi_cross_query, indi_output=config.indi_cross_output))
            
        self.layer.append(T5LayerFF(config))
        
        if config.is_decoder:
            self.next_token_layer = nn.ModuleList()
            #code for proposed methods
            if config.share_self_attention_module:
                print("shared self attention")
                self.next_token_layer.append(self.layer[0])
            else:
                print("not shared self attention")
                self.next_token_layer.append(T5LayerSelfAttention(config, has_relative_attention_bias=has_relative_attention_bias))
                
            if config.share_cross_attention_module:
                print("shared cross attention")
                self.next_token_layer.append(self.layer[1])
            else:
                print("not shared cross attention")
                self.next_token_layer.append(T5LayerCrossAttention(config))
            ...
            if config.share_ffnn:
                ...
                self.next_token_layer.append(T5LayerFF(config))
    ...
    def forward(
        ...
        #code for proposed methods
        if self.is_decoder:
            next_token_self_attention_outputs = self.next_token_layer[0](
            ...
            next_token_hidden_states, next_token_present_key_value_state = next_token_self_attention_outputs[:2]
            next_token_attention_outputs = next_token_self_attention_outputs[2:]  # Keep self-attention outputs and relative position weights
        ###
        ...
        if do_cross_attention:
            ...
            #code for proposed methods
            next_token_cross_attention_outputs = self.layer[1](
                next_token_hidden_states,
            ...
            next_token_hidden_states = next_token_cross_attention_outputs[0]
            next_token_hidden_states = self.next_token_layer[-1](next_token_hidden_states)
            if next_token_hidden_states.dtype == torch.float16:
                clamp_value = torch.where(
                    torch.isinf(next_token_hidden_states).any(),
                    torch.finfo(next_token_hidden_states.dtype).max - 1000,
                    torch.finfo(next_token_hidden_states.dtype).max,
                )
                next_token_hidden_states = torch.clamp(next_token_hidden_states, min=-clamp_value, max=clamp_value)
            ###
        ...
        #code for proposed methods
        outputs = outputs + (next_token_hidden_states,)
        ###
    ...
    def forward(
        ...
        if self.is_decoder:
            if self.config.next_token_type == "new_token":
                ...
                next_token_hidden_embeds = self.embed_tokens(new_token_ids)
        ...
        for i, (layer_module, past_key_value) in enumerate(zip(self.block, past_key_values)):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = layer_module(
                    hidden_states,
                    next_token_hidden_states=next_token_hidden_states,
            ...
            next_token_hidden_states = layer_outputs[-1]
        ...
        #code for proposed methods
        if self.is_decoder:
            next_token_hidden_states = self.final_layer_norm(next_token_hidden_states)
            next_token_hidden_states = self.dropout(next_token_hidden_states)
        ...
        if self.is_decoder:
            return BaseModelOutputWithPastAndCrossAttentions(
                last_hidden_state=next_token_hidden_states,
